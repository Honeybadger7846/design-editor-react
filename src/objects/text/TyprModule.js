/* eslint-disable */
var Typr = {};
var windowContext = global ? global : window
...
Typr["B"] = {
    ...
    _tdec: windowContext["TextDecoder"] ? new windowContext["TextDecoder"]() : null,
...
Typr["T"].CFF = {
    ...
    readIndex: function (data, offset, inds) {
        ...
        if (offsize == 1)
            ...
            for (var i = 0; i < count; i++) inds.push(bin.readUshort(data, offset + i * 2));
        else if (offsize == 3)
            for (var i = 0; i < count; i++) inds.push(bin.readUint(data, offset + i * 3 - 1) & 0x00ffffff);
        else if (offsize == 4)
            for (var i = 0; i < count; i++) inds.push(bin.readUint(data, offset + i * 4));
...
Typr["T"].loca = {
    parseTab: function (data, offset, length, font) {
        ...
        if (ver == 0)
            for (var i = 0; i < len; i++) obj.push(bin.readUshort(data, offset + (i << 1)) << 1);
        if (ver == 1)
            for (var i = 0; i < len; i++) obj.push(bin.readUint(data, offset + (i << 2)));
...
Typr["T"].name = {
    parseTab: function (data, offset, length) {
        ...
        for (var i = 0; i < count; i++) {
            ...
            var str;
            if (false) {} else if (platformID == 0) str = bin.readUnicode(data, soff, slen / 2);
        ...
        */
        ...
        for (var p in obj)
            if (obj[p][psn] != null) return obj[p];
...
Typr["U"] = {
    "CheckForMissingBitmaps": function (font, g) {
        var SVG = font["SVG "];
        var U = Typr["U"];
        if (SVG && SVG.entries[g]) {
            var p = SVG.entries[g];
            if (p != null) {
                if (typeof p == "string") {
                    U["SVG"].toPath(p, font, g)
                }
            }
        }
    },
    ...
    "shapeToPath": function (font, shape, clr) {
        ...
        for (var i = 0; i < shape.length; i++) {
            ...
            for (var j = 0; j < crds.length; j += 2) {
                tpath.crds.push(crds[j] + it["dx"]);
                tpath.crds.push(crds[j + 1] + it["dy"]);
            }
    ...
    "codeToGlyph": function (font, code) {
        ...
        if (gid != 0 && font["CFF "] == null && (SVG == null || SVG.entries[gid] == null) && loca[gid] == loca[gid + 1] // loca not present in CFF or SVG fonts
            ...
            ].indexOf(code) == -1 && !(0x2000 <= code && code <= 0x200a)) gid = 0;
        //*/
    ...
    "pathToSVG": function (path, prec, shouldApplyTransform, transform) {
        ...
        if (shouldApplyTransform && transform) {
            for (var j = 0; j < crds.length; j += 2) {
                crds[j] = crds[j] * transform.scaleX + transform.x
                crds[j + 1] = crds[j + 1] * transform.scaleY + transform.y
            }
        }
        var out = [],
            co = 0,
    ...
    "pathToContext": function (glyph, ctx) {
        var c = 0,
            cmds = glyph.path["cmds"],
            crds = glyph.path["crds"];
        //console.log(glyph.path)
        var scale = 1 / glyph.font.head.unitsPerEm * glyph.styles.fontSize
        var position = {
            x: -glyph.textSize.width / 2 + glyph.x,
            y: glyph.textSize.height / 2 - glyph.y
        }
        var usedFillCmd = false
        ctx.save()
        ctx.scale(1, -1) // fonts have inverted y axis
        ctx.translate(position.x, position.y)
        for (var j = 0; j < cmds.length; j++) {
            var cmd = cmds[j];
            //console.log(cmd)
            if (cmd == "M") {
                ctx.moveTo(crds[c] * scale, crds[c + 1] * scale);
                c += 2;
            } else if (cmd == "L") {
                ctx.lineTo(crds[c] * scale, crds[c + 1] * scale);
                c += 2;
            } else if (cmd == "C") {
                ctx.bezierCurveTo(crds[c] * scale, crds[c + 1] * scale, crds[c + 2] * scale, crds[c + 3] * scale, crds[c + 4] * scale, crds[c + 5] * scale);
                c += 6;
            } else if (cmd == "Q") {
                ctx.quadraticCurveTo(crds[c] * scale, crds[c + 1] * scale, crds[c + 2] * scale, crds[c + 3] * scale);
                c += 4;
            }
            // check if it's svg color
            else if (typeof cmd === 'object') {
                // check if it's gradient
                if (typeof cmd.fill === 'object') {
                    if (cmd.fill.type === 'linearGradient') {
                        // + cmd.fill.gradientTransform[4]) * cmd.fill.gradientTransform[0]
                        /*
                        x1: (parseFloat(cmd.fill.x1) + cmd.fill.gradientTransform[4]) / cmd.fill.gradientTransform[0] * scale,
							y1: (parseFloat(cmd.fill.y1) + cmd.fill.gradientTransform[5]) / cmd.fill.gradientTransform[3] * scale,
							x2: (parseFloat(cmd.fill.x2) + cmd.fill.gradientTransform[4]) / cmd.fill.gradientTransform[0] * scale,
							y2: (parseFloat(cmd.fill.y2) + cmd.fill.gradientTransform[5]) / cmd.fill.gradientTransform[3] * scale,
                        */
                        //console.log(cmd.fill.gradientTransform)
                        //console.log(cmd.fill.gradientTransform[4] / cmd.fill.gradientTransform[0])
                        var coords = {
                            x1: ((parseFloat(cmd.fill.x1) * cmd.fill.gradientTransform[0]) + cmd.fill.gradientTransform[4]) * scale,
                            y1: ((-parseFloat(cmd.fill.y1) * cmd.fill.gradientTransform[3]) - cmd.fill.gradientTransform[5]) * scale,
                            x2: ((parseFloat(cmd.fill.x2) * cmd.fill.gradientTransform[0]) + cmd.fill.gradientTransform[4]) * scale,
                            y2: ((-parseFloat(cmd.fill.y2) * cmd.fill.gradientTransform[3]) - cmd.fill.gradientTransform[5]) * scale,
                        }
                        var gradient = ctx.createLinearGradient(coords.x1, coords.y1, coords.x2, coords.y2)
                        for (var i = 0; i < cmd.fill.stops.length; i++) {
                            var stopColor = cmd.fill.stops[i]['stop-color']
                            if (typeof cmd.fill.stops[i]['stop-opacity'] !== 'undefined' && cmd.fill.stops[i]['stop-opacity'] !== '1') {
                                stopColor = new fabric.Color(stopColor).setAlpha(parseFloat(cmd.fill.stops[i]['stop-opacity'])).toRgba()
                            }
                            gradient.addColorStop(parseFloat(cmd.fill.stops[i]['offset']), stopColor)
                        }
                        ctx.fillStyle = gradient
                    } else if (cmd.fill.type === 'radialGradient') {
                        var coords = {
                            x1: ((parseFloat(cmd.fill.fx) * cmd.fill.gradientTransform[0]) + cmd.fill.gradientTransform[4]) * scale, // x start circle
                            y1: ((-parseFloat(cmd.fill.fy) * cmd.fill.gradientTransform[3]) - cmd.fill.gradientTransform[5]) * scale, // y start circle
                            r1: parseFloat(cmd.fill.fr) * scale, // radius start
                            x2: ((parseFloat(cmd.fill.cx) * cmd.fill.gradientTransform[0]) + cmd.fill.gradientTransform[4]) * scale, // x end circle
                            y2: ((-parseFloat(cmd.fill.cy) * cmd.fill.gradientTransform[3]) - cmd.fill.gradientTransform[5]) * scale, // y end circle
                            r2: parseFloat(cmd.fill.r) * scale, // radius end
                        }
                        var gradient = ctx.createRadialGradient(coords.x1, coords.y1, coords.r1, coords.x2, coords.y2, coords.r2)
                        for (var i = 0; i < cmd.fill.stops.length; i++) {
                            var stopColor = cmd.fill.stops[i]['stop-color']
                            if (typeof cmd.fill.stops[i]['stop-opacity'] !== 'undefined') {
                                stopColor = new fabric.Color(stopColor).setAlpha(parseFloat(cmd.fill.stops[i]['stop-opacity'])).toRgba()
                            }
                            gradient.addColorStop(parseFloat(cmd.fill.stops[i]['offset']), stopColor)
                        }
                ...
                ctx.globalAlpha = (glyph.textOpacity || 1) - 1 / cmd.opacity / 100 * (glyph.textOpacity || 1)
                ctx.beginPath()
            } else if (cmd == "Z") {
                ctx.closePath();
            } else if (cmd == "X") {
    ...
    "SVG": function () {
        ...
        function toPath(str, font, gid) {
            if (typeof str === 'string') {
                str = str.replace(/xlink:href=/gi, 'href=')
            }
            var pth = {
                cmds: [],
                crds: [],
            ...
            // parse gradients if any
            pth.cmds.forEach(cmd => {
                if (typeof cmd == 'object') {
                    if (/url/i.test(cmd.fill)) {
                        var id = /\(([^)]+)\)/.exec(cmd.fill)
                        if (Array.isArray(id) && id.length > 1) {
                            // id parsing could be improved in future
                            id = id[1].replace(/"/gi, '').trim()
                            id = id.slice(1, id.length)
                        }
                        if (id) {
                            var gradient = document.getElementById(id)
                            //console.log(gradient)
                            if (gradient && gradient.tagName == 'linearGradient') {
                                var stops = []
                                var gradientTransform = readTrnf(gradient.getAttribute('gradientTransform') || windowContext.getComputedStyle(gradient).getPropertyValue('gradientTransform') || [1, 0, 0, 1, 0, 0])
                                //console.log(gradientTransform)
                                var parentGroups = []
                                var gradientElRef = gradient
                                while (gradientElRef) {
                                    if (gradientElRef.tagName === 'g') {
                                        parentGroups.unshift(gradientElRef);
                                    }
                                    if (gradientElRef.tagName === 'svg') {
                                        break;
                                    };
                                    gradientElRef = gradientElRef.parentNode;
                                }
                                parentGroups.forEach(parent => {
                                    var parentTransform = readTrnf(parent.getAttribute('transform') || windowContext.getComputedStyle(parent).getPropertyValue('transform') || [1, 0, 0, 1, 0, 0])
                                    //console.log(parent.getAttribute('transform'))
                                    //console.log(readTrnf(parent.getAttribute('transform')))
                                    gradientTransform = [parentTransform[0] * gradientTransform[0], parentTransform[1] * gradientTransform[1], parentTransform[2] * gradientTransform[2], parentTransform[3] * gradientTransform[3], parentTransform[4] + gradientTransform[4], parentTransform[5] + gradientTransform[5]]
                                })

                                for (var i = 0, children = gradient.children; i < children.length; i++) {
                                    stops.push({
                                        'offset': children[i].getAttribute('offset') || windowContext.getComputedStyle(children[i]).getPropertyValue('offset') || 0,
                                        'stop-color': children[i].getAttribute('stop-color') || windowContext.getComputedStyle(children[i]).getPropertyValue('stop-color') || '#000000',
                                        'stop-opacity': children[i].getAttribute('stop-opacity') || windowContext.getComputedStyle(children[i]).getPropertyValue('stop-opacity') || 1
                                    })
                                }
                                cmd.fill = {
                                    id: gradient.getAttribute('id') + Math.random() * (99999 - 11111) + 11111,
                                    type: gradient.tagName,
                                    gradientUnits: gradient.getAttribute('gradientUnits') || windowContext.getComputedStyle(gradient).getPropertyValue('gradientUnits'),
                                    x1: gradient.getAttribute('x1') || windowContext.getComputedStyle(gradient).getPropertyValue('x1') || 0,
                                    y1: gradient.getAttribute('y1') || windowContext.getComputedStyle(gradient).getPropertyValue('y1') || 0,
                                    x2: gradient.getAttribute('x2') || windowContext.getComputedStyle(gradient).getPropertyValue('x2') || 0,
                                    y2: gradient.getAttribute('y2') || windowContext.getComputedStyle(gradient).getPropertyValue('y2') || 0,
                                    gradientTransform: gradientTransform,
                                    stops: stops
                                }
                            } else if (gradient && gradient.tagName == 'radialGradient') {
                                var stops = []
                                var gradientTransform = readTrnf(gradient.getAttribute('gradientTransform') || windowContext.getComputedStyle(gradient).getPropertyValue('gradientTransform') || [1, 0, 0, 1, 0, 0])
                                var parentGroups = []
                                var gradientElRef = gradient
                                ...
                                parentGroups.forEach(parent => {
                                    var parentTransform = readTrnf(parent.getAttribute('transform') || windowContext.getComputedStyle(parent).getPropertyValue('transform') || [1, 0, 0, 1, 0, 0])
                                    gradientTransform = [gradientTransform[0] * parentTransform[0], gradientTransform[1] * parentTransform[1], gradientTransform[2] * parentTransform[2], gradientTransform[3] * parentTransform[3], gradientTransform[4] + parentTransform[4], gradientTransform[5] + parentTransform[5]]
                                })
                                ...
                                cmd.fill = {
                                    ...
                                    type: gradient.tagName,
                                    gradientUnits: gradient.getAttribute('gradientUnits') || windowContext.getComputedStyle(gradient).getPropertyValue('gradientUnits'),
                                    cx: gradient.getAttribute('cx') || windowContext.getComputedStyle(gradient).getPropertyValue('cx') || 0,
                                    cy: gradient.getAttribute('cy') || windowContext.getComputedStyle(gradient).getPropertyValue('cy') || 0,
                                    fr: gradient.getAttribute('fr') || windowContext.getComputedStyle(gradient).getPropertyValue('fr') || 0,
                                    fx: gradient.getAttribute('fx') || windowContext.getComputedStyle(gradient).getPropertyValue('fx') || gradient.getAttribute('cx') || windowContext.getComputedStyle(gradient).getPropertyValue('cx') || 0,
                                    fy: gradient.getAttribute('fy') || windowContext.getComputedStyle(gradient).getPropertyValue('fy') || gradient.getAttribute('cy') || windowContext.getComputedStyle(gradient).getPropertyValue('cy') || 0,
                                    r: gradient.getAttribute('r') || windowContext.getComputedStyle(gradient).getPropertyValue('r') || 0,
                                    gradientTransform: gradientTransform,
                                    stops: stops
                                }
                            }
            ...
            // check if we have image elements
            if (pth.images && pth.images.length > 0) {
                font.bitmapTable = font.bitmapTable ? font.bitmapTable : {}
                font.bitmapTable[gid] = font.bitmapTable[gid] ? font.bitmapTable[gid] : pth.images
            }
        ...
        function _toPath(nds, pth, parent) {
            for (var ni = 0; ni < nds.length; ni++) {
                ...
                if (tn == "g") {
                    ...
                    _toPath(nd.children, tp, nd)
                    // here we apply group transform to path
                    ...
                    pth.images = pth.images.concat(tp.images)
                } else if (tn == "image") {
                    var parentTransform = readTrnf(parent && parent.getAttribute('transform') || [1, 0, 0, 1, 0, 0])
                    var imageTransform = readTrnf(nd.getAttribute('transform') || [1, 0, 0, 1, 0, 0])
                    var image = new Image()
                    image.src = nd.getAttribute('href')
                    image.width = parseInt(nd.getAttribute('width')) * parentTransform[0] * imageTransform[0] || 0
                    image.height = parseInt(nd.getAttribute('height')) * parentTransform[3] * imageTransform[3] || 0
                    image.translateX = (parseInt(nd.getAttribute('x')) || 0) + parentTransform[4] + imageTransform[4]
                    image.translateY = (parseInt(nd.getAttribute('y')) || 0) + parentTransform[5] + imageTransform[5]
                    image.opacity = parseFloat(nd.getAttribute('opacity') || windowContext.getComputedStyle(nd).getPropertyValue('opacity') || 1)
                    pth.images.push(image)
                } else if (tn == "path" || tn == "circle" || tn == "ellipse" || tn == "polygon") {
                    pth.cmds.push({
                        fill: nd.getAttribute('fill') || windowContext.getComputedStyle(nd).getPropertyValue('fill') || '#000000',
                        opacity: nd.getAttribute('opacity') || windowContext.getComputedStyle(nd).getPropertyValue('opacity') || 1,
                    })
                    var d;
                    ...
                    if (tn == "polygon") {
                        d = ["M", nd.getAttribute("points"), "Z"].join(" ");
                    }
                    svgToPath(d, pth)
                    pth.cmds.push("X")
                }
