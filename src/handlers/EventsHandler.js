...
class EventsHandler extends BaseHandler {
  constructor(props) {
    ...
    this.onMouseDown = (e) => {
      ...
      this._lastPosX =
        e.e.touches && e.e.touches[0] ? e.e.touches[0].clientX : e.e.clientX;
      this._lastPosY =
        e.e.touches && e.e.touches[0] ? e.e.touches[0].clientY : e.e.clientY;
      this._middleClick =
        e.e.which === 2 || (e.e.touches && e.e.touches[0] && !e.target);
    };
    this.onMouseMove = (event) => {
      const isMiddleClick =
        event.e.which === 2 ||
        (event.e.touches && event.e.touches[0] && !event.target);
      let pointerPos = {
        x:
          event.e.touches && event.e.touches[0]
            ? event.e.touches[0].clientX
            : event.e.clientX,
        y:
          event.e.touches && event.e.touches[0]
            ? event.e.touches[0].clientY
            : event.e.clientY,
      };
      if (isMiddleClick) {
        this.canvas.selection = false;
        this.handlers.zoomHandler.relativePan(
          new fabric.Point(
            pointerPos.x - this._lastPosX,
            pointerPos.y - this._lastPosY
          )
        );
      } else {
        this.canvas.selection = true;
      }
      this._lastPosX = pointerPos.x;
      this._lastPosY = pointerPos.y;
    };
    ...
    this.onMouseWheel = (event) => {
      ...
      if (isCtrlKey) {
        ...
        this.handlers.zoomHandler.relativePan(
          new fabric.Point(-event.e.deltaX / 2, -event.e.deltaY / 2)
        );
      }
    ...
    this.handleSelection = (target) => {
      if (target) {
        ...
        const isNotMultipleSelection =
          (initialSelection && initialSelection.type === "group") ||
