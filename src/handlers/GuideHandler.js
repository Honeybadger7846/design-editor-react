...
class GuideHandler extends BaseHandler {
    constructor(props) {
        super(props);
        this.viewportTransform = [];
        this.initAligningGuide(this.canvas);
    }
    initAligningGuide(canvas) {
        var ctx = canvas.getSelectionContext(), aligningLineOffset = 5, aligningLineMargin = 4, aligningLineWidth = 1.5, aligningLineColor = '#3a86ff', viewportTransform, zoom = canvas.getZoom();
        ...
        canvas.on('after:render', function () {
            for (let i = vertical.length; i--;) {
                drawVerticalLine(vertical[i]);
            }
            for (let i = horizontal.length; i--;) {
                drawHorizontalLine(horizontal[i]);
            }
            vertical.length = horizontal.length = 0;
        });
