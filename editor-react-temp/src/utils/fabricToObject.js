...
class ExportObject {
    ...
    [ObjectType.STATIC_TEXT](item, options, inGroup) {
        ...
        const metadata = Object.assign(Object.assign({}, item.metadata), { angle,
            fill,
            fontWeight, charspacing: charSpacing, fontSize: fontSize, template: text, fontFamily,
            textAlign, lineheight: lineHeight, text: item.text });
        const object = Object.assign(Object.assign({}, baseOptions), { metadata });
        return object;
    }
    [ObjectType.DYNAMIC_TEXT](item, options, inGroup) {
        const baseOptions = this.getBaseOptions(item, options, inGroup);
        const { fontFamily, textAlign, fontSize, fontWeight, charSpacing, lineHeight, fill, angle, originalText, keyValues } = item;
        const metadata = Object.assign(Object.assign({}, item.metadata), { angle,
            fill,
            fontWeight,
            charSpacing,
            fontSize, text: originalText, fontFamily,
            textAlign,
            lineHeight,
            keyValues, keys: item.keys ? item.keys : [] });
        const object = Object.assign(Object.assign({}, baseOptions), { metadata });
        return object;
    }
    ...
    [ObjectType.DYNAMIC_IMAGE](item, options, inGroup) {
        ...
        const object = Object.assign(Object.assign({}, baseOptions), { width: width * scaleX, height: height * scaleY, scaleX: 1, scaleY: 1, metadata });
        return object;
    }
