...
getDirectories('./elements/').forEach(directory => {
    elementsList.push({
        name: directory,
        files: fs.readdirSync(`./elements/${directory}`, {
            withFileTypes: true
        }).map(dirent => dirent.name)
    })
})
...
getDirectories('./photos/').forEach(directory => {
    photosList.push({
        name: directory,
        files: fs.readdirSync(`./photos/${directory}`, {
            withFileTypes: true
        }).map(dirent => dirent.name)
    })
})
...
app.post('/api/svg', function (req, res) {
    ...
    if (body.page) {
        ...
        editor.loadTemplate(body.page).then(() => editor.getPDF(PDFDocument, Base64Encode, (base64) => {
            res.send(base64)
        }))
    }
})
